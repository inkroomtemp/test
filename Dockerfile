...
RUN sed -i "s@http://.*archive.ubuntu.com@http://mirrors.huaweicloud.com@g" /etc/apt/sources.list \
   ...
   && apt update -y && apt upgrade -y && apt install -y git unzip wget vim gcc cmake g++  \
...
RUN cat ../config.json | sed 's/127.0.0.1/10.0.10.3/g' | sed 's/test1/root/g' | sed 's/pass": "root"/pass": "yapi"/g' > ../config.json
